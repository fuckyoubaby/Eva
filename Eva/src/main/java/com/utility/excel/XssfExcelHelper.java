...
public class XssfExcelHelper extends ExcelHelper {
	...
	@Override
	public <T> List<T> readExcel(Class<T> clazz, String[] fieldNames,
			int sheetNo, int rowOffset, int columnOffset) throws Exception {
		List<T> dataModels = new ArrayList<T>();
		// 获取excel工作簿
		InputStream is = new FileInputStream(file);
		XSSFWorkbook workbook = new XSSFWorkbook(is);
		XSSFSheet sheet = workbook.getSheetAt(sheetNo);
		int start = sheet.getFirstRowNum() + rowOffset; // 如果有标题则从第二行开始
		int rowEnd = sheet.getLastRowNum();
		int cloumnLength = fieldNames.length + columnOffset;
		...
		for (int i = start; i <= rowEnd; i++) {
			...
			// 生成实例并通过反射调用setter方法
			T target = clazz.newInstance();
			for (int j = columnOffset; j < cloumnLength; j++) {
				String fieldName = fieldNames[j-columnOffset];
				if (fieldName == null || UID.equals(fieldName)) {
					continue; // 过滤serialVersionUID属性
				}
				// 获取excel单元格的内容
	...
	public String readExcelInPosition(CellPosition position) throws Exception {
		...
		XSSFSheet sheet = workbook.getSheet(position.getSheetName());
		XSSFCell cell =sheet.getRow(position.getRowIndex()).getCell(position.getColumnIndex());
